...
export QFLAGS := -machine q35 -m 2G -rtc base=localtime -net none -chardev stdio,id=char0,logfile=$(LOG_FILE),signal=off \
  -serial chardev:char0 -drive if=pflash,format=raw,unit=0,file="$(OVMF_DIR)/OVMF_CODE-pure-efi.fd",readonly=on -drive if=pflash,format=raw,unit=1,file="$(OVMF_DIR)/OVMF_VARS-pure-efi.fd"
